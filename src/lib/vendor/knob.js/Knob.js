...
(function (undefined) {
    ...
    */
		...
		if (inputEl.hasAttribute('data-indicator-auto-position')) {
			options.indicatorAutoPosition = parseBool(
				inputEl.getAttribute('data-indicator-auto-position')
			);
		}
		...
		if (inputEl.hasAttribute('data-sprite-separation-angle')) {
			options.spriteSeparationAngle = parseFloat(
				inputEl.getAttribute('data-sprite-separation-angle')
			);
		}
  ...
  */
    ...
    */
		...
		doTouchStart: function (touches, timeStamp) {
			...
			self.__initialAngle = angleFromCoord(
				self.__initialTouchLeft,
				self.__initialTouchTop,
				self.__centerPageX,
				self.__centerPageY
			);
		...
		doTouchMove: function (touches, timeStamp, scale) {
			...
			if (!self.__isGestureLocked) {
				...
				self.__slideXDetected =
					self.options.gestureSlideXEnabled &&
					distanceX >= minimumTrackingForChange &&
					distanceY < maximumSlideVariance;
				self.__slideYDetected =
					self.options.gestureSlideYEnabled &&
					distanceY >= minimumTrackingForChange &&
					distanceX < maximumSlideVariance;
				...
				self.__totalDistance += pointDistance(
					self.__lastTouchLeft,
					self.__lastTouchTop,
					currentTouchLeft,
					currentTouchTop
				);
    ...
    */
		...
		__valueFromAngles: function (prevAngle, nextAngle) {
			...
			if (
				isFinite(self.options.angleStart) &&
				...
				return map(
					nextAngle,
					self.options.angleStart,
					self.options.angleEnd,
					self.options.valueMax,
					self.options.valueMin
				);
			}
		...
		__angleFromValue: function (value) {
			...
			// If angle and value bounds are real, map angle directly to value
			if (
				isFinite(self.options.valueMin) &&
		...
		__getIndicator: function (angle) {
			...
			if (self.options.indicatorAutoPosition) {
				...
				((indicator.x =
					self.__centerPageX - self.__clientLeft + self.options.indicatorRadius * Math.cos(rads)),
					(indicator.y =
						self.__centerPageY - self.__clientTop - self.options.indicatorRadius * Math.sin(rads)));
			} else {
		...
		__getSpriteOffset: function (angle) {
			...
			if (self.options.spriteCount > 1) {
				...
				let imageIndex =
					Math.floor(spriteAngle / self.options.spriteSeparationAngle) % self.options.spriteCount;
				if (imageIndex < 0) {
					imageIndex += self.options.spriteCount;
				}

				offset =
					self.options.spriteSeparationGap * (imageIndex + 1) +
					self.options.spriteWidth * imageIndex;
			}
